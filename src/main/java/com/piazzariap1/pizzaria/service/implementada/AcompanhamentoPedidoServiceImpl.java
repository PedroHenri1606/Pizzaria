...
public class AcompanhamentoPedidoServiceImpl implements AcompanhamentoPedidoService {
    ...
    public AcompanhamentoPedido editar(Long id, AcompanhamentoPedidoDTO acompanhamentoNovo){
        ...
        if(id == 0 || !acompanhamentoNovo.id().equals(acompanhamentoBanco.getId())){
            throw new NaoLocalizadoException(NAO_LOCALIZADO);
        }

        acompanhamentoBanco.setQuantidade(acompanhamentoNovo.quantidade());
        acompanhamentoBanco.setObservacao(acompanhamentoNovo.observacao());
        acompanhamentoBanco.setAcompanhamento(acompanhamentoNovo.acompanhamento());
